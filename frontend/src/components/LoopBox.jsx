...
export default function LoopBox({ loop }) {
  ...
  if (loop.kind === "for_range") {
    ...
    const pct =
      total_iterations && current_iteration != null
        ? Math.min(100, Math.round(((current_iteration + 1) / total_iterations) * 100))
        : 0;
    return (
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-sm text-amber-100">
              {index_var} = {current_value ?? start} in range({start}, {stop}
              {step !== 1 ? `, ${step}` : ""})
            </span>
          </div>
          <span className="text-xs text-white/50">
            {current_iteration != null ? current_iteration + 1 : "-"}
            {total_iterations != null ? ` / ${total_iterations}` : ""}
          </span>
        </div>
  ...
  const desc =
    loop.kind === "while"
      ...
      : loop.kind === "for_enumerate"
      ? `for ${(loop.vars || []).join(", ")} in enumerate(...)`
      : `for ${(loop.vars || []).join(", ")} in ${loop.iterable || "..."}`;

  return (
    <div className="rounded-xl border border-amber-400/30 bg-amber-500/[0.07] px-4 py-2.5 flex items-center gap-2">
      <Badge color="amber">{loop.kind.replace("_", " ")}</Badge>
      <span className="font-mono text-sm text-amber-100">{desc}</span>
    </div>
  );
}
